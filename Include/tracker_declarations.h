...
namespace NTracker
{
    ...
    using trackerValueCallback_t = std::function<void(trackerNumber_t, trackerValue_t)>;
    
    // тип обратного вызова, в который передается значение датчика
    // в случае срабатывания сигнализации порогового значения.
    using trackerAlertCallback_t = std::function<checkAlertTimeout_t(trackerNumber_t, trackerValue_t)>;
