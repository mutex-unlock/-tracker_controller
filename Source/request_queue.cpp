...
void TRequestQueue::start_queue()
{
    ...
    {
        queueThread_ = std::jthread([this](std::stop_token stoken) {
            this->read_request(stoken);
        });
    }
}
...
void TRequestQueue::read_request(std::stop_token stoken)
{
    while (!stoken.stop_requested())    // C++20: пока не запрошена остановка потока
    {
        ...
        conditional_.wait(lock, [this, &stoken]()
        {
            return requestQueue_.size() > 0 || stoken.stop_requested();
        });

        while (requestQueue_.size() > 0 && !stoken.stop_requested())
        {
            TRequest cmd = requestQueue_.front();
            requestQueue_.pop();
            lock.unlock();
            cmd.callback_(cmd.trackerNumber_, cmd.trackerPointer_->get_current_value());
            lock.lock();
        }
    }
}
